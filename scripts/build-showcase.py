...
SIMPLE = [
    ...
    dict(name="structural-access", expr="[Mercury,Venus,Earth,Mars].2", pill="det · exact", out="Earth",
         cap=". structural access — the exact, OFFLINE twin of .. : pick element N of a real list (0-based, negative from end). here Earth by COUNTING the list; sol..3 reaches Earth by MEANING. same target, two access modes"),
    ...
    dict(name="vocab-compose", expr="syn={=>('reply with ONLY a comma-list of synonyms of: '++$0)}; $syn%(!'sad')", pill="llm · define a concept-op, compose it with !", out="joyful, cheerful, delighted",
         cap="Harry's ask realized — GROW the vocabulary AND compose it. define a synonyms op as a form (syn={=>…}), then compose it with the hardened ! (opposite): synonyms of the OPPOSITE of 'sad' → synonyms of 'happy'. two concept-level moves — antonym, then synonyms — snapped together in one line. numpy has no notion of opposite or synonym; a prompt won't compose them cleanly. (example output — the structure is exact, the meaning is the model's)"),
