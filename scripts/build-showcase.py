...
SIMPLE = [
    ...
    dict(name="filter", expr="$filter%({$0},[true,false,true,true])", pill="det · $filter keep-if", out="true\ntrue\ntrue",
         cap="$filter keeps the items that pass — the SELECT the family was missing. map transforms, fold reduces, scan accumulates, filter selects"),
